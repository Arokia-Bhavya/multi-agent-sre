...
`ai_platform/tools/service_matching.py`). Matching is done on word
...
- Prometheus and Jaeger port-forwarded, same as `ai_platform/tools/test_clients.py`.
...
the repo root — `ai_platform` is a real installable package (`uv sync`
installs it in editable mode), so no `PYTHONPATH` juggling is needed:

```bash
uv run pytest ai_platform/coordinator/tests    # this milestone only
...
uv run python -m ai_platform.coordinator.run_investigation --alert-name FrontendHighErrorRate --service frontend
```
