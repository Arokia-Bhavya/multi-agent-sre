...
class InvestigationGraph:
    ...
    def generate_runbook(self, alert: Dict[str, Any]) -> InvestigationState:
        ...
        than a slightly slower one. Callers that already have a *recent*
        `RCAFinding` for this exact alert/service (e.g. the copilot tool
        layer's short-TTL cache — see `copilot_tools.py`'s
        `_investigation_cache`) and are confident nothing's changed since
        can skip straight to `generate_runbook_from_finding` instead and
        avoid paying for this re-investigation.
        """
        ...
        return self.generate_runbook_from_finding(
            alert, result.get("service_name"), result["correlated_findings"], rca, base_result=result
        )

    def generate_runbook_from_finding(
        self,
        alert: Dict[str, Any],
        ...
        """
        Build a runbook directly from an already-computed `RCAFinding` and
        its correlated evidence, skipping the metrics/trace/Kubernetes
        re-fetch and the RCA LLM call that `generate_runbook` always redoes.

        This is the part of `generate_runbook` that comes after
        `investigate()` — pulled out into its own method so a caller with a
        trustworthy recent finding in hand (see `generate_runbook`'s
        docstring) can reuse it directly. `base_result` lets the caller
        supply the rest of the `InvestigationState` fields (`rca_report`,
        `metrics_findings`, etc.) to merge into the returned dict when
        available; omit it (e.g. when building the runbook purely from a
        cache entry that only kept `correlated_findings`/`rca`) and only
        `alert`/`service_name`/`correlated_findings`/`rca_finding` are set.
        """
        prompt = build_runbook_prompt(alert, service_name, correlated_findings, rca)
        messages = [
            {"role": "system", "content": RUNBOOK_SYSTEM_PROMPT},
            {"role": "user", "content": prompt},
        ]
        runbook = self._invoke_structured(messages, RunbookDoc)
        report = render_runbook_markdown(alert, service_name, rca, runbook)
        base = base_result if base_result is not None else {
            "alert": alert,
            "service_name": service_name,
            "correlated_findings": correlated_findings,
            "rca_finding": rca,
        }
        return {**base, "runbook": runbook, "runbook_report": report}
    ...
    def llm(self) -> Any:
        if self._llm is None:
            ...
            if provider == "groq":
                ...
                # Claude Sonnet 5 / 4.7+ reject any non-default temperature
                # (400 "temperature is deprecated for this model") — adaptive
                # thinking controls its own sampling, so omit the param here.
    ...
    def _invoke_structured(
        ...
        `config` is passed through to every `.invoke()` call unchanged (e.g.
        `{"callbacks": [...]}` for token/cost tracking — see
        `ai_platform/evals/run_rca_evals.py`, which calls this method
        directly rather than reimplementing its own structured-output call,
        specifically so evals get the same retry/json_mode-fallback
        resilience production traffic does instead of failing outright on
        the first malformed response).
        """
        # Only pass `config` through when actually given: existing tests
        # (and any other caller) inject simple fake structured-output
        # objects whose `.invoke()` doesn't accept a `config` kwarg at all,
        # unlike a real LangChain runnable — omitting it entirely when
        # unset keeps this method a drop-in no-op change for every existing
        # caller, not just ones that happen to tolerate an extra kwarg.
        ...
        try:
            ...
            return structured_llm.invoke(json_messages, **invoke_kwargs)
        except Exception:
