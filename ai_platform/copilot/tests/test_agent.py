...
from ai_platform.copilot.agent import CopilotReply, SRECopilot, _trim_history
...
class SRECopilotTests(TestCase):
    def setUp(self):
        ...
        # remediate_scale_deployment's service-allowlist guardrail checks
        # this before pausing for approval; "checkout" is the service used
        # by every remediation test below.
        self.kubernetes_agent.get_deployment_health.return_value = [
    ...
    # Every remediation test below scripts an evidence-gathering tool call
    # (get_deployment_health) before remediate_scale_deployment, since the
    # tool now structurally requires one earlier in the conversation — see
    # test_remediate_scale_deployment_is_blocked_without_prior_evidence for
    # the case where that's skipped.
    _EVIDENCE_CALL = AIMessage(
        content="",
        tool_calls=[{"name": "get_deployment_health", "args": {"service_name": "checkout"}, "id": "call0"}],
    )

    def test_remediate_scale_deployment_is_blocked_without_prior_evidence(self):
        responses = [
            AIMessage(
                content="",
                tool_calls=[
                    {
                        ...
                        "args": {
                            ...
                            "desired_replicas": 1,
                            "reason": "trust me, it needs it",
                        },
                        "id": "call1",
                    }
                ],
            ),
            AIMessage(content="That request was blocked by a guardrail."),
        ]
        copilot, fake_llm = self._copilot(responses)

        reply = copilot.ask("Fix the checkout deployment", thread_id="t1")

        # Blocked outright — never even reaches the human approval pause.
        self.assertIsNone(reply.pending_action)
...
class TrimHistoryTests(TestCase):
    """
    Unit tests for `_trim_history`, the `pre_model_hook` that caps how much
    conversation history is actually sent to the LLM each turn (see
    MAX_HISTORY_TOKENS's docstring for why: InMemorySaver keeps everything,
    so without this, token cost per turn grows with conversation length).
    """

    def _long_history(self, pairs: int) -> list:
        # Each pair is a human question plus a moderately long AI answer,
        # ending on a fresh HumanMessage — the shape state["messages"]
        # actually has when `pre_model_hook` runs in practice (right before
        # a model call, which is always triggered by a new human question or
        # a tool result, never left dangling on an AI message). Long enough
        # in aggregate that `pairs=40` comfortably exceeds MAX_HISTORY_TOKENS
        # (6000) under an approximate token counter.
        messages = []
        for i in range(pairs):
            messages.append(HumanMessage(content=f"question number {i} about the platform's incident history"))
            if i < pairs - 1:
                messages.append(AIMessage(content=("relevant evidence and analysis " * 30) + f"turn {i}"))
        return messages

    def test_short_history_is_returned_unchanged(self):
        messages = self._long_history(pairs=2)

        result = _trim_history({"messages": messages})

        self.assertEqual(result["llm_input_messages"], messages)

    def test_long_history_is_trimmed(self):
        messages = self._long_history(pairs=40)
    ...
    def test_trimmed_history_keeps_the_most_recent_messages(self):
        messages = self._long_history(pairs=40)

        trimmed = _trim_history({"messages": messages})["llm_input_messages"]

        # The trimmed window should end on the conversation's actual last
        # message (newest content preserved, oldest dropped first).
        self.assertEqual(trimmed[-1].content, messages[-1].content)

    def test_trimmed_history_starts_on_a_human_message(self):
        messages = self._long_history(pairs=40)

        trimmed = _trim_history({"messages": messages})["llm_input_messages"]

        # Anthropic/Groq both reject a message list that opens with a
        # dangling AI/tool message — the trimmed window must start clean.
