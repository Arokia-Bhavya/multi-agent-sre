...
embeddings-first/TF-IDF-fallback logic `search_similar_incidents` uses, so
with `VOYAGE_API_KEY` configured, a query like "why do we get 502s from
checkout" can match a playbook titled "Elevated Error Rate" purely on
meaning, not shared wording.
"""
