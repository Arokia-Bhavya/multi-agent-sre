...
def escape_label_value(value: Any) -> str:
    """
    Escape a value for safe interpolation into a PromQL label matcher, e.g.
    `{service_name="<value>"}`.

    `metrics_agent.py` and `anomaly_detector.py` both f-string a
    `service_name`/`container_name` straight into label matchers, and that
    value ultimately traces back to untrusted input — an Alertmanager
    webhook payload's `alertname`/`labels.service_name`, or an LLM tool-call
    argument the model formed from alert annotations it read. Without
    escaping, a value containing a `"` breaks out of the matcher and lets
    the rest of the string inject arbitrary PromQL (extra label matchers,
    `or`'d-in vector selectors, etc.) — read-only, since these are all
    `query`/`query_range` calls, but it can still pull data the caller
    wasn't scoped to see, or make the RCA reason about the wrong metrics.

    Follows PromQL's own string-literal escaping: backslash and
    double-quote are backslash-escaped, embedded newlines are represented
    as the two-character `\\n` rather than a real line break (a real
    embedded newline would make the PromQL parser reject the query
    outright, which is a denial of a legitimate-looking request rather than
    a clean escape).
    """
