...
#
# ARCHITECTURE.md's "Known gaps" flagged the original shared-secret
...
#
# Previously gated *every* /api/* route (see git history / ARCHITECTURE.md).
...
async def _require_auth(request: Request, call_next):
    ...
    if path == "/api/webhook/alertmanager":
        if WEBUI_API_KEY:
            ...
            # TEMP DEBUG — remove once the intermittent 401 is root-caused.
            print(
                f"[webhook-debug] client={request.client} "
                f"x-api-key={request.headers.get('x-api-key')!r} "
                f"authorization={auth_header!r} "
                f"user-agent={request.headers.get('user-agent')!r} "
                f"provided={provided!r} expected={WEBUI_API_KEY!r} "
                f"match={hmac.compare_digest(provided, WEBUI_API_KEY)}"
            )
...
def alertmanager_webhook(payload: Dict[str, Any], background_tasks: BackgroundTasks) -> Dict[str, Any]:
    ...
    """
    copilot = get_copilot()
    # TEMP DEBUG — remove once the "FrontendPodDown never opens an incident"
    # issue is root-caused. Logs exactly what Alertmanager sent (status +
    # fingerprint per alert) so we can see whether the payload never arrives
    # with this alert, or arrives but gets deduped/skipped downstream.
    for _a in payload.get("alerts", []) or []:
        print(
            f"[webhook-payload-debug] status={_a.get('status')!r} "
            f"alertname={_a.get('labels', {}).get('alertname')!r} "
            f"fingerprint={_a.get('fingerprint')!r}"
        )
    new_fingerprints = handle_alertmanager_webhook(payload, _incidents)
    print(f"[webhook-payload-debug] new_fingerprints={new_fingerprints!r}")
