...
| File | Purpose |
| --- | --- |
| `helm/otel-demo-minimal-values.yaml` | Demo app install. Disables `kafka`, `opensearch`, `postgresql`, and `llm` (which overcommit a local Kind node), defines the application alert rules (`FrontendHighErrorRate`, `FrontendHighLatency`, `FrontendHighMemoryUsage`, `FrontendPodDown`, `ProductCatalogPodDown`, `ProductCatalogHighErrorRate`, `ProductCatalogHighLatency`, `ProductCatalogHighMemoryUsage`), and enables the chart's own Alertmanager subchart, routing every firing/resolved alert to the copilot's auto-triage webhook. |
